...
std::vector<std::vector<std::set<int> > > fillLayers(const cv::Mat &image, const cv::Mat &image_Lab, const std::vector<double> &point_cloud, const std::vector<double> &normals, const std::map<int, Segment> &segments, const RepresenterPenalties &PENALTIES, const DataStatistics STATISTICS, const int NUM_LAYERS, const std::vector<int> &current_solution_labels, const int current_solution_num_surfaces, const std::map<int, std::map<int, bool> > &segment_layer_certainty_map, const bool USE_PANORAMA, const bool TOLERATE_CONFLICTS, const bool APPLY_EROSION_AND_DILATION, const std::string &image_name, const std::vector<std::vector<std::set<int> > > &additional_segment_indices_map = std::vector<std::vector<std::set<int> > >(), const bool additional_segments_activity = true, const std::set<int> &erosion_layers = std::set<int>());

std::map<int, std::map<int, bool> > swapLayers(const int IMAGE_WIDTH, const int IMAGE_HEIGHT, const std::map<int, Segment> &segments, const std::vector<int> &current_solution_labels, const int NUM_LAYERS, const DataStatistics STATISTICS, const bool USE_PANORAMA, const bool CONSIDER_ROOM_STRUCTURE_LAYER = false, const bool use_disconnnected_neighbors = false, const std::set<int> &invalid_segments = std::set<int>());
...
std::set<int> findBehindRoomStructureSegments(const int IMAGE_WIDTH, const int IMAGE_HEIGHT, const std::map<int, Segment> &segments, const std::set<int> &new_segment_indices, const std::vector<std::set<int> > &pixel_segment_indices_map, const DataStatistics STATISTICS, const bool USE_PANORAMA);
